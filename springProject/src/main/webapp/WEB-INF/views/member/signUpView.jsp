	...
	<div class="container">
		...
		$name.on("input", function(){
			...
			if(!regExp.test($name.val())) {
				$("#checkName").text("한글 두 글자 이상 입력").css("color", "red");
		...
		function validate(){
			...
			// 모든 입력값이 유효할 경우 입력된 전화번호, 주소를 하나의 문자열로 합쳐
			// form 태그 끝에 hidden 타입으로 추가하기
			// 이유 : 커맨드 객체를 이용하여 파라미터를 한 번에 받을 수 있도록 하기 위함
			$memberPhone = $("<input>", {type : "hidden", 
										 name : "memberPhone", 
										 value : $("#phone1").val() + "-" + $("#phone2").val() + "-" + $("#phone3").val()});
			
			$memberAddress = $("<input>", {type : "hidden", 
										 name : "memberAddress", 
